...
@cli.command("clean")
@click.option("--min-messages", default=3, show_default=True,
              help="Delete sessions with fewer than this many user messages")
@click.option("--project-dir", type=click.Path(), default=None,
              help="Clean only sessions for this project directory (default: all projects)")
@click.option("--dry-run", is_flag=True, help="Show what would be deleted without deleting")
@click.option("-v", "--verbose", is_flag=True)
def cmd_clean(min_messages, project_dir, dry_run, verbose):
    """Delete stub/empty sessions left by aborted Claude Code starts.

    By default removes sessions with fewer than 3 user messages across all
    projects. Use --project-dir to scope to one project, --min-messages to
    adjust the threshold.
    """
    if dry_run:
        console.print("  [yellow]Dry run — no files will be deleted.[/yellow]")

    from .backends.claude_code import PROJECTS_DIR, _read_session_metadata, _decode_project_path
    from .pathmap import encode_project_path

    if not PROJECTS_DIR.exists():
        console.print("[yellow]No Claude Code sessions found.[/yellow]")
        return

    # Determine which project storage dirs to scan
    if project_dir:
        encoded = encode_project_path(project_dir)
        scan_dirs = [PROJECTS_DIR / encoded]
    else:
    ...
    for storage_dir in scan_dirs:
        if not storage_dir.exists():
            continue
        decoded_path = _decode_project_path(storage_dir.name)

        for jsonl_path in sorted(storage_dir.glob("*.jsonl")):
            info = _read_session_metadata(jsonl_path, decoded_path)
            user_count = 0
            if info:
                # message_count includes both user and assistant; recount user only
                ...
                try:
                    ...
                    continue
            # info is None for zero-message files (pure snapshots)
            if info is None or user_count < min_messages:
                if verbose or dry_run:
                    label = f"{user_count} user msg(s)" if info else "no messages"
                    console.print(
                        f"  [{'dim' if dry_run else 'red'}]{'Would delete' if dry_run else 'Deleting'}[/]"
                        f" [{label}] {_shorten_path(str(jsonl_path))}"
                    )
                if not dry_run:
                    jsonl_path.unlink()
                deleted += 1
    ...
    console.print(f"\n  {action} [red]{deleted}[/red] stub session(s), kept [green]{kept}[/green].")
